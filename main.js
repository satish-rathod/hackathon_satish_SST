...
function showTasksList() {
  ...
  list.reverse().forEach(task => {
    const element = String.raw`
				<li class="ui segment grid equal width task" data-id=${task.id}>
					<div class="ui checkbox column">
						<input class="task-checkbox" type="checkbox" ${task.completed ? "checked" : ""}>
						<label class=${task.completed ? "task-done" : ""}>${task.text}</label>
					</div>
					<div class="column">
...
tasksList.addEventListener("click", (e) => {
  ...
  taskEl.querySelector(".task-checkbox").checked = !taskEl.querySelector(".task-checkbox").checked;
  taskEl.querySelector("label").classList.toggle("task-done");
